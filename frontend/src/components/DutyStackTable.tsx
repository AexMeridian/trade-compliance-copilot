        ...
        {lines.map((line, i) => (
          <tr key={i} className={`border-b border-hairline ${line.applies ? '' : 'text-ink-faint'}`}>
            ...
            <td className="py-2 pr-3 align-top">
              ...
              {!line.applies && line.reason_if_not_applied && (
                <div className="mt-0.5 text-xs italic text-ink-faint">{line.reason_if_not_applied}</div>
              )}
              {line.caveat && (
                <div className="mt-0.5 text-xs text-review">{line.caveat}</div>
              )}
              {line.source && (
                <div className="mt-1">
            ...
            <td className="py-2 align-top font-mono">{line.effective_date}</td>
          </tr>
        ))}
      </tbody>
      <tfoot>
