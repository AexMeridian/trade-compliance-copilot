      ...
      {steps.map((step, i) => (
        <div key={step.label} className="flex items-start gap-3">
          <div className="flex flex-col items-center">
            <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-hairline-strong font-mono text-xs text-ink-muted">
              {i + 1}
            </div>
            ...
            <div
              className={
                step.label === 'License requirement'
                  ? `mt-0.5 inline-block rounded-sm border px-2 py-0.5 font-mono text-sm ${REQUIREMENT_STYLE[step.value] ?? 'border-hairline text-ink'}`
