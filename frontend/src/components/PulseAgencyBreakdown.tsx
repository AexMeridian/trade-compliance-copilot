...
export function PulseAgencyBreakdown({ breakdown }: { breakdown: { agency: string; count: number }[] }) {
  if (breakdown.length === 0) {
    return <p className="text-sm text-ink-faint">No activity in the last 30 days.</p>;
  }
    ...
    <ul className="flex flex-col gap-2">
      {breakdown.map((b) => (
        <li key={b.agency} className="flex items-center gap-2">
          <span className="w-28 shrink-0 truncate text-xs text-ink-muted" title={AGENCY_HINTS[b.agency]}>
            {AGENCY_LABEL[b.agency] ?? b.agency}
          </span>
          <span className="h-3 flex-1 bg-hairline">
            <span className="block h-full bg-hue-indigo" style={{ width: `${Math.max((b.count / max) * 100, 4)}%` }} />
          </span>
