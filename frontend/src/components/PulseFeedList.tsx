...
function GroupMeta({ a }: { a: PulseAction }) {
  return (
    <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 tabular-nums text-xs text-ink-faint">
...
export function PulseFeedList({ actions }: { actions: PulseAction[] }) {
  if (actions.length === 0) {
    return <p className="text-sm text-ink-faint">No actions in range yet. Try refreshing.</p>;
  }
        ...
        if (g.items.length === 1) {
          ...
          return (
            <li key={a.document_number} className="border-b border-hairline pb-4 last:border-0 last:pb-0">
              <GroupMeta a={a} />
              <a href={a.html_url} target="_blank" rel="noreferrer" className="mt-1.5 block text-base leading-snug text-ink no-underline hover:text-accent">
                {a.title}
              </a>
              {a.abstract && <p className="mt-1 line-clamp-2 text-sm text-ink-muted">{a.abstract}</p>}
          ...
          <li key={g.title} className="border-b border-hairline pb-4 last:border-0 last:pb-0">
            <details>
              <summary className="cursor-pointer list-none [&::-webkit-details-marker]:hidden">
                ...
                <span className="mt-1.5 block text-base leading-snug text-ink hover:text-accent">
                  {g.title} <span className="text-sm text-ink-faint">&times;{g.items.length}</span>
                </span>
                <p className="mt-1 text-sm text-ink-faint">
                  {g.items.length} separate actions under this same notice type. The Federal Register doesn't expose distinguishing detail beyond the linked
                  document itself. Tap to list each one.
                </p>
              </summary>
                ...
                {g.items.map((a) => (
                  <li key={a.document_number}>
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex gap-3 tabular-nums text-xs text-ink-muted no-underline hover:text-accent"
                    >
